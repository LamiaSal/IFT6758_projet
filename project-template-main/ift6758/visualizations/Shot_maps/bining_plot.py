...
class advance_plot():
    ...
    def get_contour(self,team_name:str,visible:bool = False):
        ...
        team_data = self.df[['x_coord_bin','y_coord_bin',team_name]]
        ...
        z_smooth = team_data[team_name]
        #z_smooth = gaussian_filter(z.T,self.sigma)       

        x = team_data['x_coord_bin'] #np.array(range(-100,101))
        y = team_data['y_coord_bin'] #np.array(range(-50,51))
        ...
        contour =  go.Contour(
                ...
                y = y,
                z= z_smooth,
                colorscale=px.colors.sequential.RdBu, #colorscale, #px.colors.sequential.RdBu_r,#'RdBu',
                zmin=0,
                zmax=m,
                zmid=-m,
                line_smoothing=1,
        ...
        contour =  go.Contour(
                ...
                y = y,
                z= z_smooth,
                colorscale=colorscale, #colorscale, #px.colors.sequential.RdBu_r,#'RdBu',
                zmin=0,
                zmax=2,
                line_smoothing=1,
                reversescale=True,
