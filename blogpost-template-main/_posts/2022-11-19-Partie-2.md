...
Le modèle ne prédit plus que des tirs, mais performe moins bien qu'avec le sur-échantillonnage, surement parce que l'on perd de l'information en jetant autant de tir à la poubelle.
À date, la technique de sur-échantillonnage semble plus prometteuse.
...
Par contre, il y a en général 10 fois moins de buts que de tirs ratés. Alors pour améliorer davantage les prédictions nous allons tenter de trouver le meilleur seuil de prédiction basé sur le f1_score.
...
On remarque aussi que le XGB et le réseau neuronal ont des résultats similaires que obtenus sur l'ensemble de validation, ce qui signifie que les deux modèles généralisent bien.
