...
Reponse : 
Pour tous les évenements on a des infomations sur les joueurs qui sont concernés pas l'action dans l'attribut "players" et en particulier leur ID. On pourrait identifier l'équipe de chaque joueur à partir de leur ID, en regardant dans gameData->players-> < IDdu joueur > -> currentTeam. Puis, compter le nombre de joueurs appartenants à chaque team pour connaitre ensuite avoir le rapport de force réel.
...
Reponse : 
1. Est ce que un tir a été assisté ou pas => on regardes dans event->players et si il y en a un avec le type playerType.

2. Est ce que le goal est un point décisif (ie ça a departagé les 2 équipes d'une égalité. event-> about -> goals. Si ya égalité oui.

3. (est ce que c'est une golden goal fait à la période 4/5/6 etcc).
...
les shotmaps permettent d'indiquer les espaces que chaque équipe a plus exploité en moyenne que les autres équipes. De ces graphiques on peut donc y lire et analyser les stratégies des différentes équipes. Est ce qu'une équipe a plus tendance à tirer depuis la droite du filet ou plutôt à gauche. Ce sont des éléments importants car en tant qu'équipe adverse, on pourrait alors apprendre à contrer ce genre de tactiques.
